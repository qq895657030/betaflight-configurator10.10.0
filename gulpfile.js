    ...
    return rollup
        .rollup({
            ...
            input: {
                ...
                'js/tabs/msp_log': 'src/js/tabs/msp_log.js',
...
function buildNWApps(platforms, flavor, dir, done) {
    if (platforms.length > 0) {
        ...
        if (!fs.existsSync(cachePath)) {
            ...
            const exePath = path.join(cachePath, exeName);
            if (!fs.existsSync(exePath)) {
                console.error(`ERROR: Executable not found at ${exePath}`);
                process.exit(1);
            }
            console.log(`✅ Local cache verified: ${exePath} found.`);
        }
        // --- 修改结束 ---
        ...
        const builder = new NwBuilder(Object.assign({
            ...
            platforms,
            flavor,
            cacheDir: './cache', // 显式指定缓存目录
            // 关键 Hack: 某些版本的 nw-builder 如果检测到 files 存在且 cacheDir 正确，
